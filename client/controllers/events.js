...
myApp.controller('EventsController', ['$scope', '$http', '$location', '$routeParams','moment', 'calendarConfig', function($scope, $http, $location, $routeParams,moment,  calendarConfig){
    ...
    if(isNotMobile)
        $scope.calendarView = 'month'; //calender set to month if desktop view
    else
        $scope.calendarView = 'week';//calender set to week if mobile view
    ...
    var actions = [{
        ...
        onClick: function (args) {
            $scope.removeEvent(args.calendarEvent.id);
        }
    }];
